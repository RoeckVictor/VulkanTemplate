...
namespace VulkanTutorial
{
	...
	void SimpleModelApp::Setup()
	{
		...
		// [COMMENT] The descriptor set layout used to pass uniform values (per frame data) to the shader
		// [COMMENT] This will be the set of indice 0
		auto globalSetLayout = DescriptorSetLayout::Builder(device)
			// [COMMENT] The uniform buffer is bound to binding 0
			.AddBinding(0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_ALL_GRAPHICS)
			// [COMMENT] The sampler is bound to binding 0
		...
		for (int i = 0; i < globalDescriptorSets.size(); i++)
		{
			...
			DescriptorWriter(*globalSetLayout, *globalPool)
				...
				// [COMMENT] Write the texture data to binding 0
